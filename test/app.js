...
class China extends Component {
	...
	provinceOption() {
		return this.jsonData.map((item, index) => {
			return <option key={index} data-index={index}>{item.name}</option>
		});
	}
	...
	cityOption() {
		if (this.index.provinceIndex == -1) {
			...
			return this.jsonData[this.index.provinceIndex].city.map((item, index) => {
				return <option key={index} data-index={index}>{item.name}</option>
			});
		}
	...
	countyOption() {
		if (this.index.cityIndex == -1) {
			...
			return this.jsonData[this.index.provinceIndex].city[this.index.cityIndex].area.map((item, index) => {
				return <option key={index} data-index={index}>{item}</option>
			});
		}

	}
	handleChangeProvince(e) {
		this.index.provinceIndex = e.target.options[e.target.selectedIndex].getAttribute("data-index");
	...
	handleChangeCity(e) {
		this.index.cityIndex = e.target.options[e.target.selectedIndex].getAttribute("data-index");
		this.setState({
			county: this.countyOption()
		});
	...
	render() {
		...
		return (
			...
			<select onChange={this.handleChangeCity} ref='city'>
				<option key="-1" value="-1" data-index="-1">地级市</option>
				{city}
			</select>
			<select onChange={this.handleChangeCounty} ref='county'>
...
ReactDOM.render(<China />,document.getElementById("app"));