...
function CoordinateFactory(triggerNode, popoverNode, direction, align){
    ...
    CF.triggerRect = CF.getBoundingClientRect(); 
    CF.popoverRect = CF.getBoundingClientRect();
    CF.direction = CF.getComputedDerection(direction);
    CF.align = CF.getComputedAlign(align);
    CF.center = [
        CF.triggerRect.left + CF.popoverRect.width/2 + scrollLeft ,
        CF.triggerRect.top + CF.popoverRect.height/2 + scrollTop
    ]
...
Object.assign(CoordinateFactory,{
    ...
    align: null,
    center: null,
    getComputedDerection(direction) {
        const {triggerRect, popoverRect} = this
        if(direction === 'up' || direction === 'down') {
            if(triggerRect.top > popoverRect.height ){
                direction = 'down'
            } else if( triggerRect.top + triggerRect.height + popoverRect.height > window.innerHeight) {
                direction = 'up'
            }
        }
        return direction
